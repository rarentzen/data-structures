...
public class BinaryTree<T> implements Tree<T>
{
	...
	public BinaryTree(T obj)
	{
		...
		root = new BinaryNode<T>(obj, null, null);
	}
	 ...
	 */
	protected BinaryNode<T> getNode(T obj)
	{
	 ...
	 */
	public boolean addLeft(T obj, T parent)
	{
		// new node with new item
		BinaryNode<T> childNode = new BinaryNode<T>(obj);
	 ...
	 *	@param - parent		the item to which the new item should be added
	 */
	public boolean addRight(T obj, T parent)
	{
		// new node with new item
		BinaryNode<T> childNode = new BinaryNode<T>(obj);
	 ...
	 */
	private String toString(BinaryNode<T> node, int level)
	{
		String s = "";
		...
		else
		{
			...
			while (itr.hasNext())
			{
				...
				if (node.left == null)
				{
					...
					node.left = new BinaryNode<T>(obj);
				...
				else if (node.right == null)
				{
					...
					node.right = new BinaryNode<T>(obj);
	...
	private abstract class BinaryTreeIterator implements TreeIterator<T>
	{
		 ...
		 *********/
 		 ...
 		 */
 		protected abstract BinaryNode<T> getCurrentNode();
 		...
 		{	
			...
			BinaryNode<T> parent = getParentNode(node);
 		...
 		/**
 	 	 ...
 	 	 */
 		protected BinaryNode<T> getParentNode(BinaryNode<T> node)
 		{
 			if (node == root)
 			{
 		...
 		public T getCurrent()
 		{
 			...
 			BinaryNode<T> node = getCurrentNode();
		...
		public boolean add(T obj)
		{
			...
			// retrieve current node in traversal
			BinaryNode<T> current = getCurrentNode();
			
			
			// create new node containing obj
			BinaryNode<T> newNode = new BinaryNode<T>(obj);
	...
	private class PreOrderIterator extends BinaryTreeIterator
	{
		...
		protected Stack<BinaryNode<T>> s;
	...
	private class LevelOrderIterator extends BinaryTreeIterator
	{
		...
		/** stores nodes yet to be traversed */
		private ArrayQueue<BinaryNode<T>> q;
		 ...
		 *	iteration begins with the root node
		 */
		public LevelOrderIterator(BinaryTree<T> tree)
		{
			// invoke constructor of superclass, BinaryTreeIterator
 			super(tree);
 			
 			// initialize queue
 			q = new ArrayQueue<BinaryNode<T>>();
		...
		public void next()
		{
			...
			BinaryNode<T> current = q.dequeue();
		...
		public boolean remove()
		{
			...
			if (!q.isEmpty())
			{
				if (temp.equals(q.getFront().theItem))
				{
	...
	private static class BinaryNode<E>
	{
		 ...
		 */
		public BinaryNode(E item, BinaryNode<E> leftChild, BinaryNode<E> rightChild)
		{
			theItem = item;
			left = leftChild;
			right = rightChild;
