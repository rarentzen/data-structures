 ...
 * the following ordering rule: 1) items < a given item, X, will be located in
 * X's left subtree 2) items > a given item, X, will be located in X's right
 * subtree 3) duplicate items are not allowed
 *
 * Written by Roger West, Edited by Elham Buxton, University of Illinois at
 * Springfield
 */
public class GenericBinarySearchTree<T extends Comparable<? super T>>
	...
	/**
	 * create a new GenericBinarySearchTree with the specified item in the root
	 * node
	 */
	...
	/**
	 * return the height of the specified node returns -1 if node is null
	 */
	...
	/**
	 * beginning at the specified node, return the node containing the minimum
	 * item
	 *
	...
	/**
	 * beginning at the specified node, return the node containing the maximum
	 * item
	 *
	...
	/**
	 * internal, recursive method to remove an item search for item begins at
	 * the specified node
	 */
	protected BinarySearchNode<T> remove(T obj, BinarySearchNode<T> node) {
		// search for item to remove
		if (node == null) {
			// throw exception if item does not exist in this
			// GenericBinarySearchTree
	 ...
	 *
	 * returns true if removal was successful, false otherwise throws
	 * NoSuchElementException if tree is empty
	 */
	...
	/**
	 * return a String representation of the GenericBinarySearchTree items will
	...
	/**
	 * return an iterator for traversing the tree in an inorder fashion this
	 * iterator will return the items in ascending order
	 */
	...
	/**
	 * return default iterator for this GenericBinarySearchTree default iterator
	...
	/**
	 * add the specified item to the tree the item will be added such that the
	 * ordering of the tree is preserved
	 */
	...
	/**
	 * remove the first item in the GenericBinarySearchTree by default, the
	 * first item is considered to be the root item
	 */
	...
	/**
	 * inner abstract class GenericBinarySearchTreeIterator encapsulates the
	 * basic functionality of an iterator for a GenericBinarySearchTree
	 */
	protected abstract class GenericBinarySearchTreeIterator implements
			OrderedIterator<T> {
		...
		/**
		 * create a new GenericBinarySearchTreeIterator to traverse the
		 * specified GenericBinarySearchTree
		 */
		public GenericBinarySearchTreeIterator(GenericBinarySearchTree<T> tree) {
			...
			// sync mod counts of GenericBinarySearchTree and this
			// GenericBinarySearchTreeIterator
		...
		/**
		 * return the current BinarySearchNode object only accessible within
		 * GenericBinarySearchTreeIterator and its subclasses
		 */
		...
		/**
		 * return the item in the current node throws a NoSuchElementException
	 ...
	 *
	 * traverses a GenericBinarySearchTree in preorder fashion: parent --> left
	 * child --> right child
	 */
	protected class PreOrderIterator extends GenericBinarySearchTreeIterator {
		...
		/**
		 * create a new PreOrderIterator for the specified
		 * GenericBinarySearchTree iteration begins with the root node
		 */
		...
		/**
		 * return the current BinarySearchNode in the iteration return null if
		 * the stack is empty
		 */
		...
		public boolean hasNext() {
			// throw exception if BinaryTree is modified outside this
			// PreOrderIterator
	 ...
	 *
	 * traverses a GenericBinarySearchTree in postorder fashion: left child -->
	 * right child --> parent
	 */
	protected class PostOrderIterator extends GenericBinarySearchTreeIterator {
		...
		/**
		 * create a new PostOrderIterator for the specified
		 * GenericBinarySearchTree iteration begins with the root node
		 */
		...
		/**
		 * advance to the next node in the traversal throws
		 * NoSuchElementException if attempt is made to traverse to a
		 * non-existent node
		 */
		public void next() {
			...
			// empty stack and null current node means traversal has gone past
			// last item
			if (s.isEmpty()) {
			...
			for (;;) {
				...
				// if this is the 3rd time cnode has been popped, ready to
				// process cnode
				...
				// not ready to process cnode yet, so push it back onto the
				// stack
				s.push(cnode);
		...
		/**
		 * inner class ItrNode represents a node that maintains the number of
		 * times it has been popped off the stack
		 */
		protected class ItrNode {
			...
			/**
			 * return a String representation of the ItrNode by default, this
			 * method simply returns the String representation of the data item
			 * in the BinarySearchNode
			 */
			public String toString() {
	 ...
	 *
	 * traverses a GenericBinarySearchTree in inorder fashion: left child -->
	 * parent --> right child
	 */
	protected class InOrderIterator extends PostOrderIterator {
		...
		/**
		 * create a new InOrderIterator for the specified
		 * GenericBinarySearchTree iteration begins with the root node
		 */
		...
		public void next() {
			...
			for (;;) {
				...
				// 2nd time cnode has been popped; ready to process cnode; push
	 ...
	 *
	 * traverses a GenericBinarySearchTree in level-order fashion: one level at
	 * a time, beginning with root and proceeding down the tree at each level,
	...
	protected class LevelOrderIterator extends GenericBinarySearchTreeIterator {
		...
		/**
		 * create a new LevelOrderIterator for the specified
		 * GenericBinarySearchTree iteration begins with the root node
		 */
	...
	/**
	 * nested class BinarySearchNode stores a data item and references to left
	...
	private static class BinarySearchNode<E extends Comparable<? super E>> {
		...
		/**
		 * create a new BinarySearchNode containing the specified item left and
		 * right child nodes will be null
		 */
		 ...
		 */
		public BinarySearchNode(E obj, BinarySearchNode<E> leftChild,
				BinarySearchNode<E> rightChild) {
			theItem = obj;
			left = leftChild;
			right = rightChild;
		}
		...
		/**
		 * return a String representation of a BinarySearchNode by default, this
		 * method simply returns the String representation of the data item in
		 * the node
		 */
		public String toString() {
